...
def transfer_rights(filename: str, user_1: str, user_2: str, read: str, write: str,
                    execute: str):  # Нужно проверить: есть ли привилегии у пользователя на этот файл. если да,
    # то он может наделить правами на него и другого пользователя
    if user_1 == "root":
        temp_array_user_2 = []
...
def create_user():
    ...
    if login == "root" and password == "1234":
        ...
        with open("login_password.csv", 'a', newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(tmp)
        with open(login_new_user + ".csv", 'w') as csv_file:  # создание csv для нового пользователя, где будут отображаться его привилегии
            pass
    else:
        print("You are not authorized to do this.")
        executor()
